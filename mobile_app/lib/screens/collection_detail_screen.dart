...
class CollectionDetailScreen extends StatelessWidget {
  ...
  static final Uri _donationUri = Uri.parse('https://xn--80adnee0afc6kza.com');
          ...
          onPressed: () async {
            ...
            if (!launched && context.mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('Не удалось открыть сайт для пожертвования'),
                ),
              );
            }
          },
          child: const Text('ПОМОЧЬ ЧЕРЕЗ САЙТ'),
        ),
      ),
    );
  }
